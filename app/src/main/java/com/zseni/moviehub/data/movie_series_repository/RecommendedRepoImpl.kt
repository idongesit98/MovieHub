...
class RecommendedRepoImpl @Inject constructor(
    ...
    override suspend fun getRecommended(
        ...
        return flow {
            ...
            val movieListFromRemote = try {
                ...
                apiService.getRecommendedMovies(category, page,movieId)
            } catch (e: IOException) {
                e.printStackTrace()
    ...
    override suspend fun getRecommendedSeries(
        forceFetchFromRemote: Boolean,
        movieId: Int,
        category: String,
        isRefresh: Boolean,
        ...
        return flow {
            ...
            val movieListFromRemote = try {
                //bring an if else statement here if movietype is movie else tvseries //movietvlist
                apiService.getRecommendedTvShows(category, page,movieId)
            } catch (e: IOException) {
                e.printStackTrace()
                emit(Resource.Error(message = "Error loading movies"))
                return@flow
